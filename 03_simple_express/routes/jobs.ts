...
PubSub.subscribe(ProcessingStates.md5, async (_msg: ProcessingStates, data: IJob) => {
...
PubSub.subscribe(ProcessingStates.complete, async (_msg: ProcessingStates, data: IJob) => {
  logger.info(data, `${ProcessingStates.complete}`)
  data.state = ProcessingStates.complete
  await sleep(2000)
  PubSub.publish(ProcessingStates.removal, data)
})

PubSub.subscribe(ProcessingStates.removal, async (_msg: ProcessingStates, data: IJob) => {
  logger.info(data, `${ProcessingStates.removal} ${Object.keys(JOBS).length}`)
  data.state = ProcessingStates.removal
  delete JOBS[data.id]
...
const allfilesHandler = async (_request: Request, response: Response) => {
